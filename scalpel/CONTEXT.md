...
When a command arrives, the router walks this decision tree **top-down, first match wins**. Every team-owned piece plugs into one of these rungs.

### Step 0 — Is it a compound command or shell script? → tiered parser

Before anything else, check for multi-command input:

- **Control flow** (`for`, `while`, `if`, `case`, function definitions) → escalate the **entire block** to Tier 3 (AWS) immediately. The local LLM can't fake loop iteration state consistently, and splitting loops is worse than sending the whole thing up.
- **Simple chains** (`;`, `&&`, `||`, `|`, newlines with no control flow) → split with `shlex`, route each piece through this same decision tree, concatenate outputs in order.
- **Single command** → fall through to Step 1.

### Step 1 — Is it in the static lookup table? → Tier 1

Commands whose output is identical on every run of the reference Pi live in `scalpel/router/lookup_table.py`. Dict lookup, instant return, zero cost.

Populated by running `capture_truth.py` against the ref Pi and auto-extracting any command whose 5 runs produced identical `(stdout, stderr, rc)`.

Examples: `uname -m`, `arch`, `cat /etc/os-release`, `cat /etc/debian_version`, `hostnamectl`, `cat /proc/cpuinfo`.

### Step 2 — Is it in the goodLLM list? → Tier 2 (local Ollama)

Commands where the local LLM reliably produces correct-looking output **and** stays fast. Pre-validated by running Ollama against the command and diffing vs. ref Pi.

Examples: `ls`, `ps`, `cat <small dynamic file>`, `echo`, `env`, `which`, `type`, `free`.

### Step 3 — Is it in the badLLM list? → Tier 3 (AWS Bedrock)

Commands the local LLM botches **or** that are slow locally. Two overlapping reasons to land here:

- **Botches**: long structured outputs the small model hallucinates inconsistently — `dpkg -l`, `apt list --installed`, `systemctl list-units`, `find / ...` with complex predicates.
- **Slow**: any command whose real-Pi runtime exceeds ~1s. The expected slowness gives cover for the AWS round-trip (see Step 5).

**badLLM and slow both land here.** We don't separate them because the routing answer is the same.

### Step 4 — Unknown bucket → heuristic (default = local)

For commands in none of the three lists, apply these cheap signals:

- Contains `|` (pipe) → **Tier 3**
- Contains `$(...)` or backticks (command substitution) → **Tier 3**
- Contains `>` or `>>` (redirect) → **Tier 2** with session-state tracking (the redirect creates/modifies a file; state handling is already covered by cowrie's filesystem + our persistence hook)
- Expected output > ~100 lines → **Tier 3**
- Command length > 80 chars → **Tier 3**
- **Otherwise → Tier 2 (local LLM is the default for unknowns)**

### Step 5 — Tier 3 failure → fall back to Tier 2

If AWS is unreachable or times out (>3s), retry locally rather than returning an error. A mediocre-but-present response is less suspicious than silence. Cache Tier 2/3 responses per command per session so repeated calls don't drift across retries.

### Step 6 — The latency cover rule

**When we must escalate, hide it inside a command that's expected to be slow.** A 1–3s round-trip to AWS is invisible on a command that naturally takes that long on a real Pi; it's a glowing red flag on `whoami`.
...
**Combined heuristic:** route to the cheapest tier that produces a correct answer, and when we must escalate, hide it inside a command that's expected to be slow.

### Resolved design decisions

- **pwd** works out of the box via cowrie's shell — no routing needed.
- **Session state** (cwd, env, created files within a session) — cowrie's existing session tracking is sufficient; we do not need a separate `session_state.py`.
- **Session history / LLM context** — use the LLM client's default conversation-thread handling. Pass recent command/response pairs as context without a custom history layer.
- **Tier 3 timeout** — 3s; fall back to Tier 2 on timeout or error.
- **Default for unknowns** — Tier 2 (local). Escalate only when the heuristics fire.

---

## File Persistence

Attackers probe for state persistence: `touch /tmp/x`, disconnect, reconnect, expect `/tmp/x` still to be there. Cowrie's default virtual filesystem (`fs.pickle`) is loaded fresh at each session and discarded at session close, so attacker writes vanish. Red Team will notice.

**Our fix:** override the session's `closed()` method in cowrie to pickle the modified filesystem back to disk on every disconnect. This lives in cowrie source (not in `scalpel/`) because the close hook fires at the SSH-session layer, below our router.
...
- **Atomic write** via `tmp_path` + `os.replace()` — no partial-file corruption if the Pi loses power mid-write.
- **Reads `fs.pickle` path from `cowrie.cfg`** if the shell.filesystem key is set, falls back to the standard location.
- **Fires on both clean logout AND abrupt disconnect** — state survives either shutdown path.
- **Shared filesystem** — every attacker session reads/writes the same `fs.pickle`. For the hackathon (one red team probing one box) this is fine and actually helpful (continuity across sessions). Per-attacker filesystems would be out of scope.

**What this unlocks for the router:**

- `touch /foo`, `mkdir /foo`, `rm /foo`, `echo x > /foo` can be handled by cowrie's native filesystem logic and the state persists automatically.
- The Tier-2-with-state branch (Step 4 redirect handling) doesn't need a custom session-state layer on top — cowrie's fs mutation plus the persistence hook covers it.

---

## Current Build Status (as of update)

Keep this section truthful as we ship.

| Component | Status | Owner |
|-----------|--------|-------|
| File persistence (fs.pickle on disconnect) | **Built** | Cowrie/FS Lead |
| `scalpel/scripts/capture_truth.py` (ref Pi probe harness) | **Built** — PATH fix, no-banner, nc probes | Test Lead |
| `scalpel/tests/diff_pis.py` (honeypot ↔ ref Pi diff) | **Built** | Test Lead |
| `scalpel/tests/ground_truth.jsonl` | **Captured** — clean version after PATH fix | Test Lead |
| `scalpel/router/lookup_table.py` (Tier 1) | **Empty skeleton** — auto-populate from ground truth | Router Lead |
| goodLLM / badLLM lists | **Not started** — populate by testing Ollama vs. ref Pi | Ollama Lead |
| `scalpel/router/handle_command.py` (decision tree) | **Not started** | Router Lead |
| Compound-command / shell-script parser | **Not started** | Router Lead |
| `scalpel/local_llm/client.py` (Ollama wrapper) | **Not started** | Ollama Lead |
| `scalpel/aws/client.py` + Lambda | **Not started** | AWS Lead |
| `scalpel/cowrie_hook.py` (entry point) | **Not started** | Router Lead |
| SSH banner / MOTD hardening | **Not started** | Cowrie/FS Lead |
